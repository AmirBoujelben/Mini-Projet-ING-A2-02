...
import { AddingEtudForm } from "../SharedComponents/AddingEtudForm";
import { TableContentEtud } from "../SharedComponents/TableContentEtud";
export const AdminGestEtud = () => {
    ...
    setTimeout(async () => {
      try {
        ...
        const response = await fetch(
          "http://localhost:5700/admin/getAllEtudiants",
          {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
          ...
          setIsPending(false);
          console.log("Failed to fetch Etudiants");
        }
      } catch (err) {
        console.log("Cannot fetch Etudiants data", err);
      }
    }, 1000);
  };
    ...
    <div className="mt-5 border-2 border-solid border-slate-700 pb-8">
      <div className="flex justify-between py-4 bg-slate-800 mb-7 px-5">
        ...
        >
          ...
          <p className="text-sm text-white">Ajouter un nouveau Etudiants</p>
        </button>
      </div>
      <AddingEtudForm isOpen={isAddFormOpen} onClose={ClosingForm} />
      {isPending ? <SkeletonTable /> : <TableContentEtud data={Etudiants} />}
    </div>
  );
};
